...
class SlideShow3DWidget(BrainAtlasWidget):
    def __init__(self, parent = None):
        ...
        self.coords = [[40, 45, 25],
                       ...
                       [-25, -90, 0],
                       [-20, -80, 10]]
        self.slides = [self.set_degree, self.set_strength, self.set_eccentricity, self.set_global_efficiency, self.set_path_length, self.set_triangles, self.set_clustering, self.set_participation, self.set_betweenness, self.set_modularity, self.set_zscore] # add measure-functions here
    ...
    def set_triangles(self): # copy this function and alter coords and edges
        ...
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], self.color, 0.7) #specify any rgb color, last digit should be 1
        edges = [(0,3), (7,8), (8,13), (16, 19), (23, 24), (24, 29)]
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], [1.0, 1.0, 1.0, 1.0], 0.3) 
        edges = [(0,1), (0,2), (1,2), (16,17), (16,18), (17,18)]
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], [0.0, 0.0, 1.0, 1.0], 0.7)
        return description


    def set_clustering(self):
        description = 'CLUSTERING coefficient is the fraction of triangles around a node. It is equivalent to the fraction of the neighbors of the node that are neighbors of each other.'
        self.set_triangles()
        return description
    ...
    def set_degree(self): 
        ...
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], self.color, 0.7)
        return description


    def set_strength(self): 
        description = 'STRENGTH of a node is the sum of the weights of the edges connected to the node. Number of connections are ignored in calculations.'
        self.clear_animation()
        coords = self.coords
        brain_regions = []
        for c in coords:
        ...
        edges = [(0,2), (16,18)]
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], self.color, 0.7, label = '0.9')
        edges = [(2,6), (2,7), (2,12), (2,14), (2,10), (3,9), (3,4), (3,5), (3,14), (3,13), 
        (4,10), (4,9), (4,7), (5,6), (5,7), (5,15), (5,11), (5,10), (6,9), (6,10), (6,11), 
        (6,7), (6,13), (7,12), (7,14), (7,13), (9,14), (9,15), (9,13), (10, 13), (10,12), 
        (10,15), (12,15), (13,15), (14,15), 
        ...
        edges = [(8,11),
                (24,27)]
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], [0.0, 0.0, 1.0, 1.0], 0.7, label = '0.1')
        return description
    ...
    def set_eccentricity(self): 
        ...
        for c in green_coords:
            self.add_region(c, [0.0, 1.0, 0.0, 1.0], 5)
    ...
    def set_path_length(self):
        description = 'PATH LENGTH of a node is the average path length from that note to all other nodes. If more than one path exist between a node and other node, the shortest path is chosen (red path in the figure).'
        self.set_eccentricity()
        return description


    def set_global_efficiency(self):
        description = 'GLOBAL EFFICIENCY is the average inverse shortest path length in the graph. It is inversely related to the characteristic path length. If more than one path exist between a node and other node, the shortest path is chosen (red path in the figure).'
        self.set_eccentricity()
        return description


    def set_participation(self):
        description = 'PARTICIPATION coefficient compares the number of links of a node has with nodes of other cluster to the number of links within its cluster. Nodes with a high participation coefficient (known as connector hubs) are connected to many clusters and are likely to facilitate global intermodular integration.'
        self.clear_animation()
        coords = self.coords
        brain_regions = []
        ...
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], [0.0, 1.0, 0.0, 1.0], 0.7) 
    ...
    def set_modularity(self):
        description = 'MODULARITY is a statistic that quantifies the degree to which the graph may be subdivided into clearly delineated groups.'
        self.clear_animation()
        coords = self.coords
        brain_regions = []
        ...
        (16,23), (19,20), (18,24), (20,24), (24,25)]
        for edge in edges:
            self.add_edge([coords[edge[0]], coords[edge[1]]], [1.0, 1.0, 1.0, 1.0], 0.7) 
        return description


    def set_betweenness(self):
        description = 'BETWEENNESS CENTRALITY of a node is the fraction of all shortest paths in the graph that contain a given node. Nodes with high values of betweenness centrality participate in a large number of shortest paths.'
        self.clear_animation()
        coords = self.coords
        brain_regions = []
        for c in coords:
        ...
        red_coords = [coords[8], coords[24]]
        for c in red_coords:
            self.add_region(c, [1.0, 0.0, 0.0, 1.0], 5)
    ...
    def set_zscore(self):
        description = 'Within-module degree Z-SCORE of a node shows how well connected a node is in a given cluster by comparing the degree of the node in that cluster to the average degree of all nodes in the same cluster. This measure requires a previously determined community structure.'
        self.clear_animation()
        coords = self.coords
        brain_regions = []
        greens = [8,10,11,12,13,14,15,
